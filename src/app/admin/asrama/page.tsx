...
import { getDaftarAsrama, getDaftarAkun, tambahAsrama, editAsrama, hapusAsrama } from "./actions";
...
export default function AsramaPage() {
  ...
  const fetchData = async () => {
    setIsFetching(true);
    const [resAsrama, resAkun] = await Promise.all([getDaftarAsrama(), getDaftarAkun()]);
    if (resAsrama.data) setAsramaList(resAsrama.data);
    if (resAkun.data) setAkunList(resAkun.data);
    setIsFetching(false);
  };
    ...
    <div className="space-y-6 max-w-6xl w-full relative">
      ...
      {notif.tampil && (
        <div className={`fixed top-5 right-5 z-100 px-5 py-3 rounded-xl shadow-lg border transition-all duration-300 flex items-center gap-3 ${
          notif.tipe === "sukses" ? "bg-green-50 border-green-200 text-green-700 dark:bg-pondok-950 dark:border-green-800 dark:text-green-400" : "bg-red-50 border-red-200 text-red-700 dark:bg-red-950 dark:border-red-900 dark:text-red-400"
        }`}>
              ...
              <tr>
                <td colSpan={5} className="p-10 text-center text-gray-500">
                  <div className="flex flex-col items-center justify-center gap-3">
                    ...
                    <span className="font-medium">Memuat data asrama...</span>
                  </div>
                </td>
              </tr>
            ) : asramaList.length === 0 ? (
